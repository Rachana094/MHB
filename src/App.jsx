...
import Stage5Trivia from './components/Stage5Trivia';
import Stage6Lantern from './components/Stage6Lantern';
import Stage7Letter from './components/Stage7Letter';

const STAGE_LABELS = ['✦', 'Wishes', 'Memories', 'Constellation', 'Trivia', 'Lantern', 'Letter'];
...
const stageBgs = [
  ...
  'radial-gradient(ellipse at 50% 80%, #2D0B40 0%, #0F0A1E 55%, #1A0A2E 100%)',
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden" style={{ background: stageBgs[stage - 1], transition: 'background 1.4s ease' }}>
      ...
      <div key={stage} className="fade-in min-h-screen">
        ...
        {stage === 5 && <Stage5Trivia onNext={next} />}
        {stage === 6 && <Stage6Lantern onNext={next} />}
        {stage === 7 && <Stage7Letter />}
      </div>
    </div>
  );
}
